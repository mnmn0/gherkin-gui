...
import { GherkinValidator, TestConfigValidator, ProjectConfigValidator } from '../utils/validation';
import { 
  IpcChannels, 
  IpcEvents, 
  ProjectConfig,
...
export class IpcService {
  ...
  private setupIpcHandlers(): void {
    ...
    ipcMain.handle('project:load-config', this.handleLoadProjectConfig.bind(this));
    ipcMain.handle('project:save-config', this.handleSaveProjectConfig.bind(this));
    ipcMain.handle('project:validate-config', this.handleValidateProjectConfig.bind(this));
  }
  ...
  private sendToRenderer<K extends keyof IpcEvents>(channel: K, data: IpcEvents[K]): void {
    if (this.webContents) {
  ...
  private async handleListSpecs(): Promise<IpcChannels['file:list-specs']['response']> {
    try {
      return await this.fileManager.listSpecifications();
    } catch (error) {
      this.sendError('FILE_LIST_ERROR', `Failed to list specifications: ${error}`, 'filesystem');
      return [];
    }
  }
  ...
  private async handleLoadSpec(
    ...
    try {
      ...
      this.sendError('FILE_LOAD_ERROR', `Failed to load specification: ${error}`, 'filesystem');
      throw error;
    }
  }
  ...
  private async handleSaveSpec(
    ...
    try {
      ...
      this.sendError('FILE_SAVE_ERROR', `Failed to save specification: ${error}`, 'filesystem');
      throw error;
    }
  }
  ...
  private async handleDeleteSpec(
    ...
    try {
      ...
      this.sendError('FILE_DELETE_ERROR', `Failed to delete specification: ${error}`, 'filesystem');
      throw error;
    }
  }
  ...
  private async handleCreateSpec(
    ...
    try {
      ...
      this.sendError('FILE_CREATE_ERROR', `Failed to create specification: ${error}`, 'filesystem');
      throw error;
    }
  }
  ...
  private async handleGenerateCode(
    ...
    try {
      ...
      this.sendError('CODE_GENERATION_ERROR', `Failed to generate code: ${error}`, 'generation');
      throw error;
    }
  }
  ...
  private async handleValidateCode(
    ...
    try {
      ...
      this.sendError('CODE_VALIDATION_ERROR', `Failed to validate code: ${error}`, 'generation');
      return {
        valid: false,
        errors: [{
  ...
  private async handleExecuteTest(
    ...
    try {
      ...
      this.sendError('TEST_EXECUTION_ERROR', `Failed to execute tests: ${error}`, 'execution');
      throw error;
    }
  }
  ...
  private async handleGetTestStatus(
    ...
    try {
      ...
      this.sendError('TEST_STATUS_ERROR', `Failed to get test status: ${error}`, 'execution');
      throw error;
    }
  }
  ...
  private async handleCancelTest(
    ...
    try {
      ...
      this.sendError('TEST_CANCEL_ERROR', `Failed to cancel test: ${error}`, 'execution');
      throw error;
    }
  }

  // Report management handlers
  private async handleListReports(): Promise<IpcChannels['report:list']['response']> {
    try {
      return await this.fileManager.listReports();
    } catch (error) {
      this.sendError('REPORT_LIST_ERROR', `Failed to list reports: ${error}`, 'filesystem');
      return [];
    }
  }
  ...
  private async handleLoadReport(
    ...
    try {
      ...
      this.sendError('REPORT_LOAD_ERROR', `Failed to load report: ${error}`, 'filesystem');
      throw error;
    }
  }
  ...
  private async handleDeleteReport(
    ...
    try {
      ...
      this.sendError('REPORT_DELETE_ERROR', `Failed to delete report: ${error}`, 'filesystem');
      throw error;
    }
  }

  // Project configuration handlers
  private async handleLoadProjectConfig(): Promise<IpcChannels['project:load-config']['response']> {
    try {
      ...
      this.sendError('CONFIG_LOAD_ERROR', `Failed to load project config: ${error}`, 'configuration');
      throw error;
    }
  }
  ...
  private async handleSaveProjectConfig(
    ...
    try {
      const validation = this.projectConfigValidator.validateProjectConfig(config);
      if (!validation.valid) {
        throw new Error(`Invalid project configuration: ${validation.errors.map(e => e.message).join(', ')}`);
      }
      ...
      this.sendError('CONFIG_SAVE_ERROR', `Failed to save project config: ${error}`, 'configuration');
      throw error;
    }
  }
  ...
  private async handleValidateProjectConfig(
    ...
    try {
      ...
      this.sendError('CONFIG_VALIDATION_ERROR', `Failed to validate project config: ${error}`, 'configuration');
      return {
        valid: false,
        errors: [{
          message: `Validation failed: ${error}`,
  ...
  private validateFileName(name: string): void {
    ...
    if (!/^[a-zA-Z0-9\-_\s.]+$/.test(name)) {
      throw new Error('Invalid file name: only alphanumeric characters, spaces, hyphens, underscores, and dots are allowed');
    }
  }
  ...
  private validateGherkinContent(content: string): void {
    ...
    if (!validation.valid && validation.errors.length > 0) {
      throw new Error(`Invalid Gherkin content: ${validation.errors.map(e => e.message).join(', ')}`);
    }
  }
  ...
  private validateTestConfig(config: any): void {
    ...
    if (!validation.valid) {
      throw new Error(`Invalid test configuration: ${validation.errors.map(e => e.message).join(', ')}`);
    }
  }

  private async saveTestReport(executionId: string, testResult: any): Promise<void> {
    try {
