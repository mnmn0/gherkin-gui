...
describe('TestRunner', () => {
  ...
  it('shows loading state', () => {
    render(<TestRunner {...defaultProps} isLoading={true} />);
  ...
  it('shows empty state when no specifications', () => {
    ...
    expect(screen.getByText('Create some Gherkin specifications to run tests')).toBeInTheDocument();
  });
  ...
  it('handles specification selection', () => {
    ...
    expect(screen.getByText('1 of 2 specifications selected')).toBeInTheDocument();
  });
  ...
  it('handles select all functionality', () => {
    ...
    expect(screen.getByText('2 of 2 specifications selected')).toBeInTheDocument();
    ...
    expect(screen.getByText('0 of 2 specifications selected')).toBeInTheDocument();
  });
  ...
  it('manages configuration settings', () => {
    ...
    const jvmInput = screen.getByPlaceholderText('Add JVM argument (e.g., -Xmx1g)');
  ...
  it('prevents execution without selected specifications', () => {
    ...
    expect(window.alert).toHaveBeenCalledWith('Please select at least one specification to execute.');
