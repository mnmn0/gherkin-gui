...
// Mock the CSS import
jest.mock('../renderer/components/Specifications/SpecificationList.css', () => ({}));
...
describe('SpecificationList', () => {
  ...
  it('shows loading state', () => {
    render(<SpecificationList {...defaultProps} isLoading={true} />);
  ...
  it('shows empty state when no specifications', () => {
    ...
    expect(screen.getByText('Create your first Gherkin specification')).toBeInTheDocument();
  });
  ...
  it('handles search input', () => {
    ...
    render(<SpecificationList {...defaultProps} onSearchChange={onSearchChange} />);
  ...
  it('filters specifications based on search term', () => {
    ...
    expect(screen.getByText('user-login.feature')).toBeInTheDocument();
    expect(screen.queryByText('user-registration.feature')).not.toBeInTheDocument();
  });

  it('highlights selected specification', () => {
    render(<SpecificationList {...defaultProps} selectedSpec={mockSpecifications[0]} />);
    
    const selectedItem = screen.getByText('user-login.feature').closest('.spec-item');
  ...
  it('handles keyboard navigation', () => {
    ...
    const firstSpec = screen.getByText('user-login.feature').closest('.spec-item');
  ...
  it('sorts specifications by name', () => {
    ...
    render(<SpecificationList {...defaultProps} specifications={unsortedSpecs} />);
